...
/**
 * Дерево: ствол + двухъярусная ромбовидная крона в духе PS1 (исходная форма).
 * Размер варьируется, плюс редкий крупный вариант — великан над лесом.
 * Каждое ~восьмое дерево — неоново-фиолетовое: кибер-готика.
 */
fun DrawScope.drawTree(cx: Float, cy: Float, seed: Int) {
    var s = 0.85f + (seed and 0x7) * 0.06f            // 0.85..1.27 — обычный разброс
    if ((seed and 0x1C0) == 0) s *= 1.6f              // редкий крупный экземпляр
...
fun DrawScope.drawVegetation(
    ...
    // Клумба: органичное скопление вокруг смещённого от центра клетки ядра
    val pcx = cx + (((bits and 0x1F) - 15).toFloat()) * 0.9f; bits = bits ushr 5
    val pcy = cy + (((bits and 0x7) - 3).toFloat()) * 1.1f; bits = bits ushr 3
    val heads = 4 + (bits and 0x3) + (if (nearWater) 3 else 0); bits = bits ushr 2
    val palette = (seed ushr 13) and 0x7   // палитра общая на клумбу — читается цельно

    // Рисуем от дальних (высоких/задних) к ближним, чтобы перекрытие смотрелось глубоко.
    // Каждый цветок — овал разного размера на стебле разной высоты над биомом.
    for (i in 0 until heads) {
        val ox = (((bits and 0x1F) - 15).toFloat()) * 1.3f; bits = bits ushr 5
        val oy = (((bits and 0xF) - 7).toFloat()) * 1.0f; bits = bits ushr 4
        val gx = pcx + ox
        val gy = pcy + oy                                  // точка стебля на земле
        val stalk = (5f + (bits and 0x7)) * life; bits = bits ushr 3   // высота цветка над биомом
        val hx = gx
        val hy = gy - stalk
        var head = flowerColor(palette, bits and 0x1); bits = bits ushr 1
        if (wilting) head = head.copy(alpha = 0.6f)
        // Овал: ширина/высота слегка разные и варьируются — «кружечки разной высоты»
        val rw = (3.2f + (bits and 0x3) * 1.1f) * life; bits = bits ushr 2
        val rh = rw * (0.75f + (bits and 0x3) * 0.12f); bits = bits ushr 2

        // Стебель
        drawLine(Color(0xFF2F5D46).copy(alpha = head.alpha), Offset(gx, gy), Offset(hx, hy), strokeWidth = 1.4f)
        // Головка-овал
        drawOval(head, topLeft = Offset(hx - rw, hy - rh), size = Size(rw * 2f, rh * 2f))
    }
}
